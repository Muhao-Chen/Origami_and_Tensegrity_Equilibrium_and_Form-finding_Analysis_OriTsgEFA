function [fig_out] = tenseg_plot_ori_dash( N,C_b,C_s,C_h,C_rh,fig_handle,highlight_nodes,view_vec, PlotTitle, R3Ddata,Ca)
% /* This Source Code Form is subject to the terms of the Mozilla Public
% * License, v. 2.0. If a copy of the MPL was not distributed with this
% * file, You can obtain one at http://mozilla.org/MPL/2.0/.
%
% [fig_out] = TENSEG_PLOT_ORI_DASH(N,C_b,C_s,C_h,C_rh,fig_handle,highlight_nodes,view_vec, PlotTitle, R3Ddata,Ca)
% creates a rough visualization figure for a given origami-tensegrity structure
%
% Inputs:
...
hingeWidth=1; % Width of hinges
rgdhingeWidth=1;  % Width of rgd hinges
%% Labeling options
...
if nargin < 7+2 % Empty Plot Title
    PlotTitle = '';
end
if nargin < 8+2 % No 3D object data
    R3Ddata = [];
end
...
%% Plot origami edge C_h
if ~isempty(C_h)
    B = N*C_h';
    bar_start_nodes = zeros(3,size(B,2));
    bar_end_nodes = zeros(3,size(B,2));
    for j = 1:size(B,2)
        bar_start_nodes(:,j) = N(:,C_h(j,:)==-1);
        bar_end_nodes(:,j) = N(:,C_h(j,:)==1);
    end
    ...
    if ~isempty(R3Ddata) && isfield(R3Ddata,'Bradius') % 3D plot
        ...
        quiver3(bar_start_nodes(1,:),bar_start_nodes(2,:),bar_start_nodes(3,:),B(1,:),B(2,:),B(3,:),'black.','Autoscale','off','LineWidth',hingeWidth)
        hold on
    end
    % Write bar labels
    if LabelBars == 1
...
%% Plot origami rigid hindge C_rh
if ~isempty(C_rh)
    B = N*C_rh';
    bar_start_nodes = zeros(3,size(B,2));
    bar_end_nodes = zeros(3,size(B,2));
    for j = 1:size(B,2)
        bar_start_nodes(:,j) = N(:,C_rh(j,:)==-1);
        bar_end_nodes(:,j) = N(:,C_rh(j,:)==1);
    end
...
%         quiver3(bar_start_nodes(1,:),bar_start_nodes(2,:),bar_start_nodes(3,:),B(1,:),B(2,:),B(3,:),'y-.','Autoscale','off','LineWidth',rgdhingeWidth)
       quiver3(bar_start_nodes(1,:),bar_start_nodes(2,:),bar_start_nodes(3,:),B(1,:),B(2,:),B(3,:),'black-.','Autoscale','off','LineWidth',rgdhingeWidth,'ShowArrowHead','off')
        hold on
    end
    % Write bar labels
    if LabelBars == 1
...
%% Plot triangle plate using 'patch'- Ca
num_rbd=size(Ca,2);
for i=1:num_rbd
   N2=N(:,Ca(:,i));
